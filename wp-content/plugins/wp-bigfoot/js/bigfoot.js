	...
	$.bigfoot2 = function(options) {
		...
		var settings = $.extend(
			{
				...
				contentMarkup 		: "<aside class=\"footnote2-content bottom\"" +
											"data-footnote2-identifier=\"{{FOOTNOTENUM}}\" " +
											"alt=\"Footnote Source {{FOOTNOTENUM}}\">" +
												"<div class=\"footnote2-main-wrapper\">" +
													"<div class=\"footnote2-content-wrapper\">" +
														"{{FOOTNOTECONTENT}}" +
												"</div></div>" +
											"<div class=\"tooltip\"></div>" +
										"</aside>",

				buttonMarkup 		:  "<a href=\"#\" class=\"footnote2-button\" " +
											"data-footnote-identifier=\"{{FOOTNOTENUM}}\" " +
											"alt=\"See Footnote Source {{FOOTNOTENUM}}\" " +
											"rel=\"footnote2\"" +
											"data-footnote2-content=\"{{FOOTNOTECONTENT}}\">" +
												"<span class=\"footnote2-circle\" data-footnote2-identifier=\"{{FOOTNOTENUM}}\"></span>" +
												"<span class=\"footnote2-circle\"></span>" +
												"<span class=\"footnote2-circle\"></span>" +
										"</a>"
			}, options);
		...
		var footnoteInit = function() {
			...
			var $footnoteAnchors = $(footnoteButtonSearchQuery).filter(function() {
				...
				return ($this.attr("href") + relAttr).match(/(fn|footnote2|note2)[:\-_\d]/gi) && $this.closest("[class*=footnote2]:not(a):not(sup)").length < 1;
			}); // End of footnote link .filter()
			...
			// Get the footnote that the link was pointing to
			$(footnoteLinks).each(function() {
				relatedFN = $(this).attr("data-footnote2-ref").replace(":", "\\:");
				$closestFootnoteLi = $(relatedFN).closest("li");
				if($closestFootnoteLi.length > 0) {
					footnotes.push($closestFootnoteLi);
					finalFNLinks.push(this);
			...
			for(var i = 0; i<footnotes.length; i++) {
				...
				// Removes any backlinks and hackily encodes double quotes and >/< symbols to prevent conflicts
				var footnoteContent = removeBackLinks($(footnotes[i]).html().trim(), $(finalFNLinks[i]).data("footnote2-backlink-ref"))
										.replace(/"/g, "&quot;").replace(/&lt;/g, "&ltsym;").replace(/&gt;/g, "&gtsym;"),
					$footnoteNum = +(i + 1),
					footnoteButton = "",
					$footnoteButton;
		...
		function cleanFootnoteLinks($footnoteAnchors, footnoteLinks) {
			...
			$footnoteAnchors.each(function() {
				...
				if($supParent.length > 0) {
					...
					footnoteLinks.push(
						$supParent.attr({
							"data-footnote2-backlink-ref": linkID,
							"data-footnote2-ref": linkHREF
						})
					);
				} else if($supChild.length > 0) {
					linkID = ($supChild.attr("id") || "") + ($this.attr("id") || "");
		...
		var buttonHover = function(e) {
			if(settings.activateOnHover) {
				var $buttonHovered = $(e.target).closest(".footnote2-button"),
					dataIdentifier = "[data-footnote2-identifier=\"" + $buttonHovered.attr("data-footnote2-identifier") + "\"]";
				if($buttonHovered.hasClass("active")) return;
				...
				// Delete other popovers, unless overriden in the settings
				if(!settings.allowMultipleFN) {
					var otherPopoverSelector = ".footnote2-content:not(" + dataIdentifier + ")";
					removePopovers(otherPopoverSelector);
				}
				createPopover(".footnote2-button" + dataIdentifier).addClass("hover-instantiated");
			}
		}
		...
		var touchClick = function(e){
			var $target			= $(e.target),
				$nearButton		= $target.closest(".footnote2-button");
				$nearFootnote	= $target.closest(".footnote2-content");
		...
		var clickButton = function($button) {
			...
			// Get the identifier of the footnote
			var dataIdentifier = "data-footnote2-identifier=\"" + $button.attr("data-footnote2-identifier") + "\"";
			...
			if($button.hasClass("changing")) {
				...
				createPopover(".footnote2-button[" + dataIdentifier + "]");
				$button.addClass("click-instantiated");

				// Delete all other footnote popovers if we are only allowing one
				if(!settings.allowMultipleFN) {
					removePopovers(".footnote2-content:not([" + dataIdentifier + "])");
				}
				...
				if(!settings.allowMultipleFN) {
					...
					removePopovers(".footnote2-content[" + dataIdentifier + "]");
				}
		...
		var createPopover = function(selector) {
			...
			selector = selector || ".footnote2-button";

			// Activate all matching if multiple footnotes are allowed
			// Or only the first matching element otherwise
			if(settings.allowMultipleFN) {
				var $buttons = $(selector).closest(".footnote2-button");
			} else {
				var $buttons = $(selector + ":first").closest(".footnote2-button");
			}
			...
			$buttons.each(function() {
				...
				try {
					// Gets the easy replacements out of the way (try is there to ignore the "replacing undefined" error if it's activated too freuqnetly)
					var content = settings.contentMarkup
								.replace(/\{\{FOOTNOTENUM\}\}/g, $this.attr("data-footnote2-identifier"))
								.replace(/\{\{FOOTNOTECONTENT\}\}/g, $this.attr("data-footnote2-content").replace(/&gtsym;/, "&gt;").replace(/&ltsym;/, "&lt;"));
					...
					if(!settings.appendPopoversTo) {
						// Insert content after next block-level element, or after the nearest footnote
						$nearestBlock = $this.closest("p, div, pre, li, ul, section, article, main, aside");
						$siblingFootnote = $nearestBlock.siblings(".footnote2-content:last");
						if($siblingFootnote.length > 0) {
							$content = $(content).insertAfter($siblingFootnote);
						} else {
							$content = $(content).insertAfter($nearestBlock);
					...
					// Bind the scroll handler to the popover
					$content.find(".footnote2-content-wrapper").bindScrollHandler();
				}
			});

			// Get all footnotes activated by this function
			var $allFootnotesActivated = $(selector.replace(".footnote2-button", ".footnote2-content"));
		...
		$.fn.bindScrollHandler = function() {
			...
			$(this).on("DOMMouseScroll mousewheel", function(e) {
				...
				var $this = $(this),
					...
					scrollHeight = $this[0].scrollHeight,
					height = parseInt($this.css("height")),
					$popover = $this.closest(".footnote2-content");
		...
		var unhoverFeet = function(e) {
			if(settings.deleteOnUnhover && settings.activateOnHover) {
				setTimeout(function() {
					// If the new element is NOT a descendant of the footnote button
					var $target = $(e.target).closest(".footnote2-content, .footnote2-button");
					if($(".footnote2-button:hover, .footnote2-content:hover").length < 1) {
						removePopovers();
					}
				}, settings.hoverDelay);
			}
		...
		function removePopovers(footnotes, timeout) {
			footnotes = footnotes || ".footnote2-content";
			timeout = timeout || settings.popoverDeleteDelay;

			$(footnotes).each(function() {
				var $linkedButton = $(".footnote2-button[data-footnote2-identifier=\"" + $(this).attr("data-footnote2-identifier") + "\"]"),
					$this = $(this);
				if($linkedButton.hasClass("changing")) return;
				$linkedButton.removeClass("active hover-instantiated click-instantiated").addClass("changing");
				$this.removeClass("active").addClass("disapearing");
			...
			return $(footnotes.replace(".footnote2-content", ".footnote2-button"));
		}
		...
		var repositionFeet = function() {
			if(settings.positionContent) {
				...
				$(".footnote2-content").each(function() {

					// Element Definitions
					var $this 				= $(this),
						dataIdentifier 		= "data-footnote2-identifier=\"" + $this.attr("data-footnote2-identifier") + "\"",
						$contentWrapper 	= $this.find(".footnote2-content-wrapper"),
						$button 			= $(".footnote2-button[" + dataIdentifier + "]");
					...
					// Sets the max height so that there is no footnote overflow
					$this.find(".footnote2-content-wrapper").css({"max-height": Math.min(maxHeightOnScreen, maxHeightInCSS) + "px"});
					...
					// Give scrollable class if the content hight is larger than the container
					if(parseInt($this.css("height")) < $this.find(".footnote2-content-wrapper")[0].scrollHeight) {
						$this.addClass("scrollable");
					}
				});
			}
		...
		$(document).ready(function() {
			...
			$(document).on("mouseenter", ".footnote2-button", buttonHover);
			$(document).on("touchend click", touchClick);
			$(document).on("mouseout", ".hover-instantiated", unhoverFeet);
			$(document).on("keyup", escapeKeypress);
			$(window).on("scroll resize", repositionFeet);
