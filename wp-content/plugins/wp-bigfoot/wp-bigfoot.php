...
class WP_Bigfoot	{
	...
	function init() {
		...
		add_shortcode('footnote2', array($this,'shortcode_footnote2') );
        add_filter( 'the_content', array($this, 'the_content' ), 12 );
		add_filter( 'the_content', array($this, 'the_content2' ), 12 );
...
*/
	...
	function shortcode_footnote2( $atts, $content=NULL ){
        global $id;
		if ( null === $content )	return;
		$content = $this->remove_crappy_markup( $content );
		if ( ! isset( $this->footnotes2[$id] ) ) $this->footnotes2[$id] = array();
		$this->footnotes2[$id][] = $content;
		$count = count( $this->footnotes2[$id] );
		return '<a href="#footnote2-' . $count . '-' . $id . '" ' . 'id="note2-' . $count . '-' . $id . '" ' . 'rel="footnote2">' . $count . '</a>';
	}
	...
	function the_content2($content) {
        return $this->get_footnotes2( $content );
	}
	...
	function get_footnotes2( $content ) {
        global $id;
        if ( empty( $this->footnotes2[$id] ) )	return $content;
		$footnotes = $this->footnotes2[$id];
		if( count($footnotes) ){
			$content .= '<div class="footnotes2">';
			$content .= '<hr />';
			$content .= '<ol>';
			foreach ( $footnotes as $number => $footnote ): 
				$number++;
				$content .= '<li id="footnote2-'.$number.'-'.$id.'" class="footnote2">';
				$content .= '<p>';
				$content .= $footnote;
				$content .= '<a href="#note2-'.$number.'-'.$id.'" class="footnote2-return">&#8617;</a>';
				$content .= '</p>';
				$content .= '</li><!--/#footnote2-'.$number.'.footnote-->';
			endforeach;
			$content .= '</ol>';
			$content .= '</div><!--/#footnotes2-->';
		}
        return $content;
	}
